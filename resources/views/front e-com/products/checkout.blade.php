...
@foreach($deliveryAddress as $addresses)		
		 ...
		 <td>
				<div class="control-group" style="float:left; margin-top: -2px; margin-right: 5px;" >
					<input type="radio" id="address{{$addresses['id']}}" name="address_id" value="{{$addresses['id']}}" shipping_charges="{{$addresses['shipping_charges']}}" total_price="{{$total_price}}" coupon_amount="{{Session::get('couponAmount')}}">
				</div>
				<div class="control-group">
...
<?php Session::put('grand_total', $grand_total); ?>
 ...
 <div>
 	...
 	<input type="radio" name="payment_method" id="payumoney" value="payumoney"><strong>Payumoney</strong>
 </div>
				</div>
				</div>
				</td>
